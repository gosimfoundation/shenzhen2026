   ...
   if (hasSubLinks) {
      ...
      item.addEventListener('click', (e) => {
         ...
         if (hasSubLinks) {
            ...
            const toggle = item.querySelector(':scope > button');
            if (toggle) toggle.setAttribute('aria-expanded', String(item.classList.contains('active')));
         }
      });
   }
});
...
/**
 * Shared liquid selection indicators. The indicator is one composited element
 * per control group, so schedule and navigation state changes stay smooth
 * without running blur filters across every card.
 */
function initLiquidGooey() {
   ...
   function bindIndicator(group, itemSelector, defaultSelector, followHover) {
      ...
      const moveTo = (item, instant) => {
         ...
         const previousRect = currentItem instanceof HTMLElement
   ...
   document.querySelectorAll('.main-nav-bar nav[aria-label="Navigation Desktop"] > ul').forEach((group) => {
      bindIndicator(group, ':scope > li[data-barba-update]', null, true);
   });

   document.querySelectorAll('.preview-sidebar').forEach((group) => {
   ...
   document.querySelectorAll('.sched-sidebar .track-group').forEach((group) => {
      bindIndicator(group, '.sched-track-btn', '.active', true);
   });
   document.querySelectorAll('.mobile-track-pills, .room-day-nav, .grid-day-tabs').forEach((group) => {
      bindIndicator(group, 'button', '.active', true);
   });
...
function initBasicFunctions() {
   if (!window.__liquidHeaderControls) {
   function syncOverlayState() {
      const navOpen = $('[data-navigation-status]').attr('data-navigation-status') === 'active';
      const filterOpen = $('[data-nav-filter-status]').attr('data-nav-filter-status') === 'active';
      document.body.setAttribute('data-liquid-overlay-open', String(navOpen || filterOpen));
      $('[data-navigation-toggle="toggle"]').attr('aria-expanded', String(navOpen));
      $('[data-filter-toggle="toggle"]').closest('button').attr('aria-expanded', String(filterOpen));
   }
